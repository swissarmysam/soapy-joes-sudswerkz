...
export default class Producer extends Entity {
  constructor(opts) {
    ...
    this.state.processingEnabled =
      typeof opts.processingEnabled === 'boolean'
        ? opts.processingEnabled
        : true;
  ...
  calculateCost(count) {
    ...
    if (this.state.baseCost) {
      ...
      for (let i = 0; i < count; i++) {
        cost.price += Math.round(
          this.state.baseCost.amount *
            Math.pow(this.state.costCoefficient, this.state.count + i)
        );
      }
    }
    return cost;
  ...
  addOutput(outputType, outputKey, productionTime, productionAmount) {
    ...
    this.outputs[outputType][outputKey] =
      this.outputs[outputType][outputKey] || {};
      ...
      Object.keys(this.inputs).map(cat => {
        Object.keys(this.inputs[cat]).map(input => {
          ...
          if (lastProcessed) {
            if (obj) {
              if (
                rules.consumptionTime > 0 &&
                dt - lastProcessed >= rules.consumptionTime
              ) {
                const consumeBy = Math.min(
                  obj.count,
                  this.state.count *
                    rules.consumptionAmount *
                    Math.trunc((dt - lastProcessed) / rules.consumptionTime)
                );
    ...
    const processOutputs = () => {
      const inputRequirementsMet = reqs => {
        ...
        for (const rc of reqs) {
          if (
            this.state.consumedInputs[rc.category] &&
            this.state.consumedInputs[rc.category][rc.key]
          ) {
            if (
              this.state.consumedInputs[rc.category][rc.key].amount < rc.amount
            ) {
              return false;
            }
          } else {
      ...
      const clampByConsumedInputs = (count, reqs) => {
        ...
        for (const rc of reqs) {
          const maxConsumable = Math.min(
            count * rc.amount,
            this.state.consumedInputs[rc.category][rc.key].amount
          );
        ...
        Object.keys(this.outputs).map(cat => {
          Object.keys(this.outputs[cat]).map(output => {
            ...
            if (lastProcessed) {
              if (obj) {
                if (
                  rules.productionTime > 0 &&
                  dt - lastProcessed >= rules.productionTime
                ) {
                  if (inputRequirementsMet(rules.inputRequirements)) {
                    ...
                    const timeMultiple = Math.trunc(
                      (dt - lastProcessed) / rules.productionTime
                    );
                    // clamp to the minimum possible consumable inputs
                    const clampedCount = clampByConsumedInputs(
                      this.state.count * timeMultiple,
                      rules.inputRequirements
                    );
                    const incrementBy = clampedCount * rules.productionAmount;

                    reduceConsumpedInputsBy(
                      clampedCount,
                      rules.inputRequirements
                    );
